package me.dashengzhang.popularmovies.service;
...
public class PopularMoviesService extends IntentService {
    ...
    public static final String SORTING_EXTRA = "sorting_extra";
    ...
    protected void onHandleIntent(Intent intent) {
        mSortBy = intent.getStringExtra(SORTING_EXTRA);
        mVoteCount = intent.getStringExtra(VOTE_EXTRA);
        ...
        try {
            ...
            if (mSortBy.equals(this.getString(R.string.pref_sorting_popularity))) {
                builtUri = Uri.parse(MOVIE_BASE_URL).buildUpon()
                        ...
                        .build();
            } else if (mSortBy.equals(this.getString(R.string.pref_sorting_rating))) {
                builtUri = Uri.parse(MOVIE_BASE_URL).buildUpon()
                        .appendQueryParameter(SORTING_PARAM, mSortBy)
    ...
    private void getMovieDataFromJson(String movieJsonStr) throws JSONException {
        ...
        try {
            ...
            for (int i = 0; i < movieArray.length(); i++) {
                ...
                // poster path
                String path = this.getResources().getString(R.string.img_url_base)
                        + this.getResources().getString(R.string.img_size)
            ...
            if (cVVector.size() > 0) {
                ...
                inserted = this.getContentResolver().bulkInsert(MovieEntry.CONTENT_URI, cvArray);
            }
            Log.d(LOG_TAG, "PopularMovies Service Complete. " + inserted + " Inserted");
        } catch (JSONException e) {
            Log.e(LOG_TAG, e.getMessage(), e);
            e.printStackTrace();
        }
